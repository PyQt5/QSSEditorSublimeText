...
def is_support(name=None, syntax=None):
    if name == syntax == None:
        return False
    if name is None and str(syntax).lower().find('css') > -1:
        return True
    if not os.path.splitext(str(name))[-1] in ['.css', '.qss', '.style']:
        return False
    return True


def init_log(file=None, name=PLUGIN_NAME, level=logging.INFO, formatter=None):
    """
    初始化日志记录配置
    :param file: 文件名
    :param name: 日志名
    ...
    formatter = formatter or logging.Formatter(
        '[%(asctime)s] %(levelname)1.1s [%(module)s:%(funcName)s:%(lineno)s] %(message)s'
    )
